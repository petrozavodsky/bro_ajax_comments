...
class bro_ajax_walker_comment extends Walker_Comment {
	...
	public function html5_comment_content( $comment, $depth, $args ) {
		...
		$user_url      = get_comment_author_url( intval( $comment->comment_ID ) );
		$author_link_tag    = [
			'open'  => 'div',
			'close' => 'div'
		];

		if ( false !== filter_var( $user_url, FILTER_VALIDATE_URL ) ) {
			$author_link_tag = [
				'open'  => "a href='{$user_url}' target='blank' ",
				'close' => 'a'
			];
		}
		?>
        ...
        <article id="article-comment-<?php comment_ID(); ?>" class="comment__article">
            ...
            <div class="comment__article-item-inside">
                <div class="comment__article-item-top">
                    <<?php echo $author_link_tag['open']; ?> class="comment__article-avatar">
					<?php if ( 0 != $args['avatar_size'] ) {
						echo get_avatar( $comment, $args['avatar_size'] );
					} ?>
                </<?php echo $author_link_tag['close']; ?>><!-- .comment-author -->
            </div>

            <div class="comment__article-item-center">
                <<?php echo $author_link_tag['open']; ?> class="comment__article-author-name">
				<?php echo sprintf( '<b class="fn">%s</b>', $this->author_name_helper( $comment ) ); ?>
            </<?php echo $author_link_tag['close']; ?>>
	...
	private function author_name_helper( $comment ) {
